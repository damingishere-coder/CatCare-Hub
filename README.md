...
当前已完成 **P3｜订单系统**：项目具备可迁移的本地数据库，以及客户、多猫、订单和任务自动生成闭环。后台可以创建与编辑订单，按日期范围和每日次数生成任务，由后端统一计算费用，并同步维护订单与未执行任务的状态。
...
`/admin/orders` 作为兼容入口，会自动进入“订单计划”。按天排程、地图路线和右侧任务详情属于 P4/P5，当前尚未实现。

## 路由边界

- `/admin`：PC 管理后台
- `/admin/plans`：已实现的订单列表、详情、创建与编辑入口
- `/admin/orders`：兼容入口，重定向到 `/admin/plans`
