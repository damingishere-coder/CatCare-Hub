...
工作台默认按 Asia/Shanghai 业务日期显示：

- 当月订单：业务日期所在自然月内，至少有一个非取消服务任务的去重订单数，包含本月未来服务日期。
...
工作台批量响应只包含客户姓名、小区、猫咪数量和任务/订单摘要，不返回电话、完整地址、门禁、钥匙编号、备注或照片路径。待收提醒会直达对应订单的 P8 收款表单；“客户填写入口”打开链接管理页，今日安排和提醒下方同时显示可编辑审核稿与归档决策区。
...
后台默认生成 14 天有效的一次性专属链接，也可设置 1-90 天、复制完整微信链接，并在未提交时关闭或恢复。客户打开 `/fill/:token` 后可保存草稿；姓名/称呼必填，手机号或微信至少一项，地址、门禁方式、钥匙状态、猫咪、日期、每日次数和备注均可选。公开页不收集门禁密码、进门说明、钥匙编号、服务事项、价格、照片或文件。

客户提交后不能修改原稿。后台保留永久只读原稿，并可编辑完整审核稿、门禁、钥匙、服务事项和价格。四个明确动作分别为保存审核稿、仅归档客户、归档并生成订单和作废；每次归档始终新建客户，不搜索或合并旧客户。“仅归档客户”不会设置客户的 `archived_at`；订单模式要求地址、至少一只已命名猫咪、日期、每日次数、服务事项和单次价格，并创建已确认订单及每日任务。

公开填写页已有独立构建命令 `npm run build:public --prefix frontend`，产物位于 `frontend/dist-public/`，不包含管理后台路由。CloudBase Run 中转服务入口为 `app.intake_relay:app`，部署骨架与变量边界见 [`deploy/cloudbase/README.md`](./deploy/cloudbase/README.md)。CloudBase 资源创建、计费和真实部署必须另行确认；未配置云端中转时仍可在本机完成开发测试。

提交摘要不批量返回手机号、详细地址、门禁、钥匙或猫咪要求，只有在后台选中单条提交后才读取完整资料。Uvicorn 访问日志会将公开 API 路径中的 Token 脱敏为 `[redacted]`，公开表单的 422 错误也不会回显无效输入值。客户链接使用加密安全随机 Token、有效期和关闭状态，数据库只保存 Token 的 SHA-256 哈希；链接原文只在生成响应中显示一次，刷新后台后无法恢复，遗失时应关闭旧链接并重新生成。公开草稿、提交和审核都使用 revision/CAS；提交、认领、归档与完成回写使用持久幂等键和 Submission UUID，回写失败重试不会重复建档。
...
顶部显示今日已完成收款、待收订单数、本月已完成收款和累计完成订单。待收订单区展示客户、服务日期、猫咪数量、应收、已收和待收；按日结算订单会逐日独立计算待收与超收，不跨日期相互抵消。收款流水区展示订单项目、微信/支付宝/现金/其他方式、金额、状态和 Asia/Shanghai 时间。
...
收款流水不提供编辑或物理删除。只有 `completed` 流水可以通过“撤销误登记”改为 `voided`，系统会永久保留原金额、方式、时间，并记录必填撤销原因与撤销时间；该操作只纠正本地账目，不会发起微信、支付宝、银行卡或现金退款。收款批量响应不返回电话、微信、地址、门禁、钥匙或任何备注；登记备注仅写入本地数据库，撤销原因只用于财务审计展示。
...
中栏初次加载只读取本地缓存，不会自动把地址发送给地图厂商。用户明确点击“生成路线”后，系统才会解析缺少坐标的地址并逐段计算真实电动车路线；任务顺序使用本地快速推荐，最终总距离、预计时间和折线均来自地图 Provider。推荐可一键采用，并继续受完整日排程修订值与执行历史锁定保护。
...
3. 填写本机高德 Web 服务 Key、家庭起点经纬度；地理编码城市默认深圳市，也可覆盖。
...
```text
CATCARE_AMAP_WEB_KEY             后端地理编码和电动车路线，仅保存在本机
CATCARE_HOME_LATITUDE            路线起点纬度（GCJ-02）
CATCARE_HOME_LONGITUDE           路线起点经度（GCJ-02）
CATCARE_MAP_CITY                 地理编码城市范围，默认深圳市
...
缺少明确行政区的本地地址会在新粘贴时补充“深圳市龙岗区”；旧订单只在地图查询时临时补充，不会批量改写历史数据。点击生成路线会向地图服务发送小区、文字地址、楼栋及路线坐标；不会发送客户姓名、房号、手机号、微信、门禁、钥匙、客户备注或照片。地图请求只能从本机管理后台发起，管理入口不得对公网开放。
