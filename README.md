...
当前已完成 P14 界面重构，并按本机单人使用需求取消密码登录。管理后台和移动执行页面启动后可直接进入；客户、多猫、跨天多次订单、自动任务、排序、移动执行、图片、收款以及客户填写 Token 的完整生命周期均有自动化测试覆盖。
...
本地开发服务器会把 `/api` 代理到 `127.0.0.1:8000`。后台与移动端当前免登录，敏感响应仍禁止缓存；`start.bat` 只监听本机回环地址，绝不能将 `/admin`、`/mobile`、`/api/admin/*` 或 `/api/mobile/*` 转发到公网。
...
点击生成路线会向地图服务发送小区、文字地址、楼栋及路线坐标；不会发送客户姓名、房号、手机号、微信、门禁、钥匙、客户备注或照片。地图请求只能从本机管理后台发起，管理入口不得对公网开放。
...
任务详情会显示现场执行所需的客户联系、地址、门禁、钥匙和猫咪注意事项。页面和接口当前没有密码保护，只允许从运行 CatCare-Hub 的这台电脑访问；不要把 `5180`、`8000` 端口或后台 API 暴露到局域网或公网。
...
`/mobile` 与 `/api/mobile/*` 不再要求账号或会话。PWA 不缓存身份、任务或任何敏感业务数据；如需改为手机跨设备访问，必须先重新设计访问边界，不能直接把监听地址改回局域网。

## 界面与响应式

P14 已统一后台、移动执行端与客户填写页的视觉语言。PC 后台使用固定左侧导航与宽内容区；订单计划在 1280px 及以上保持“按天计划 + 地图路线 + 右侧任务/客户信息”三栏，在较窄窗口自然改为纵向布局。手机执行端与客户填写页支持 320px 起的窄屏，主要移动操作保持至少 44px 触控高度。

免登录改造没有改变费用计算、排程 revision、客户填写 Token、PWA 缓存或业务数据模型，也没有引入新的前端依赖。
...
禁止将真实客户资料、手机号、地址、门禁、钥匙、客户填写 Token/Payload、数据库、上传照片、`.env`、日志或本地运行文件提交到 GitHub。仓库只保存代码、配置模板和开发文档。默认只允许本机访问；安全边界见[本地免登录与私网风险说明](./docs/SECURITY_AND_PRIVATE_ACCESS.md)，绝不能把整个管理站点转发到局域网或公网。
