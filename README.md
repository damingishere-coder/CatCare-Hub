...
提交摘要不批量返回手机号、详细地址、门禁、钥匙或猫咪要求，只有在后台选中单条提交后才读取完整资料。Uvicorn 访问日志会将公开 API 路径中的 Token 脱敏为 `[redacted]`，公开表单的 422 错误也不会回显无效输入值。客户链接使用加密安全随机 Token、有效期和关闭状态，但 P12 前尚未完成 Token 哈希、防刷、正式登录和角色权限，因此仍只允许本机或可信局域网使用，不得暴露到公网。
...
禁止将真实客户资料、手机号、地址、门禁、钥匙、客户填写 Token/Payload、数据库、上传照片、`.env`、日志或本地运行文件提交到 GitHub。仓库只保存代码、配置模板和开发文档。P12 完成前，不得将 `/admin`、`/mobile`、`/fill`、`/api/admin/*`、`/api/mobile/*` 或 `/api/fill/*` 暴露到公网。
