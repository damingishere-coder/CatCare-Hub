...
P14 已统一后台、移动执行端、登录与客户填写页的视觉语言。PC 后台使用固定左侧导航与宽内容区；订单计划在 1280px 及以上保持“按天计划 + 地图路线 + 右侧任务/客户信息”三栏，在较窄窗口自然改为纵向布局。手机执行端与客户填写页支持 320px 起的窄屏，主要移动操作保持至少 44px 触控高度。

本轮只调整呈现、布局与交互层级，没有改变业务 API、费用计算、排程 revision、权限角色、PWA 缓存或数据隐私边界，也没有引入远程字体、图片、UI 框架或新依赖。
