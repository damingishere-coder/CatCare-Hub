          ...
          return (
            <div className={`min-h-24 min-w-0 border-r border-b border-slate-100 p-1.5 text-left align-top transition hover:bg-orange-50 ${selectedDate === value ? "bg-orange-50 ring-2 ring-inset ring-orange-400" : "bg-white"}`} onClick={() => onSelectDate(value)}>
              ...
              <span className="mt-1 flex flex-wrap gap-1">
                {(day?.orders ?? []).slice(0, 3).map((order) => <button key={order.order_id} type="button" className={`cc-calendar-order-marker inline-flex rounded px-1.5 py-0.5 text-[11px] font-semibold ${order.order_status === "completed" ? "bg-emerald-50 text-emerald-700" : "bg-orange-100 text-orange-900"}`} title={`订单 #${displayOrderNumber(order)} · ${order.customer_name}${order.visit_count > 1 ? ` · ${order.visit_count} 次上门` : ""}`} aria-label={`订单 #${displayOrderNumber(order)}，${order.customer_name}${order.visit_count > 1 ? `，${order.visit_count} 次上门` : ""}`} onClick={(event) => { event.stopPropagation(); onSelectOrder(order.order_id); }}>#{displayOrderNumber(order)}</button>)}
      ...
      {loading ? (
        ...
        <ol className="grid gap-2 p-3 2xl:grid-cols-2">
          {tasks.map((task, index) => (
            <li key={task.id}>
              <button type="button" className="flex w-full items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-left transition hover:border-orange-200 hover:bg-orange-50" onClick={() => onSelectOrder(task.order_id)} aria-label={`路线第 ${index + 1} 站，${task.customer.name}，订单 #${displayOrderNumber(task)}`}>
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-1 block truncate text-xs text-slate-500">订单 #{displayOrderNumber(task)} · {task.customer.address || "地址待补充"}</span>
