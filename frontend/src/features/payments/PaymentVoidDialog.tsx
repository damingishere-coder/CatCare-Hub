        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-4 p-5">
            ...
            <label className="block text-sm font-medium text-slate-700">撤销原因 <span className="text-red-600">*</span>
              <textarea ref={reasonRef} className="mt-1.5 min-h-28 w-full resize-y rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-950 outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100" value={reason} maxLength={500} onChange={(event) => setReason(event.target.value)} placeholder="例如：重复登记，实际只收到一笔" />
            </label>
