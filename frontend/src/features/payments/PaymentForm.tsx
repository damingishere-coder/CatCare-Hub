...
export function PaymentForm({ orders, initialOrderId, onCancel, onSave }: PaymentFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/35 p-4 sm:p-8" role="dialog" aria-modal="true" aria-labelledby="payment-form-title">
      <form className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white shadow-xl" onSubmit={handleSubmit}>
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4 sm:px-6">
          ...
          <button type="button" className="rounded-md p-2 text-slate-500 hover:bg-slate-100" onClick={onCancel} disabled={saving} aria-label="关闭收款表单"><X size={18} /></button>
        </div>
        ...
        <div className="flex justify-end gap-3 border-t border-slate-200 px-5 py-4 sm:px-6">
          <button type="button" className="rounded-md border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700" onClick={onCancel} disabled={saving}>取消</button>
          <button type="submit" className="inline-flex items-center gap-2 rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50" disabled={saving}>
