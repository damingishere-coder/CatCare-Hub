...
function ReviewEditor({ payload, sourceNote, unitPrice, disabled, onPayloadChange, onUnitPriceChange }: {
  ...
  return <div className="space-y-4">
    <section className="rounded-lg border border-brand-200 bg-white p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">后台审核稿</h3><p className="mt-1 text-xs text-slate-500">客户原始提交不会被覆盖；这里的修改只用于最终落档和生成订单。</p></div><label className="text-sm font-semibold text-brand-900">每次价格（元）<input className={`${reviewInputClass} w-40 border-brand-300`} type="number" min="0" step="0.01" value={unitPrice} disabled={disabled} onChange={(event) => onUnitPriceChange(event.target.value)} /></label></div>
...
export function IntakeWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  async function handleDecision(mode: IntakeDecisionMode) {
    if (!detail || action !== null || (mode === "order" && !orderReady) || (mode === "customer" && !reviewSaved)) return;
  ...
  async function handleListState(item: IntakeSubmissionSummary, removed: boolean) {
    if (selectedId === item.id && !confirmDiscard()) return;
  ...
  const humanDirty = Boolean(detail && reviewDraft && (JSON.stringify(reviewDraft) !== JSON.stringify(editableReviewPayload(detail)) || unitPrice !== (detail.review_unit_price ?? "")));
  const confirmDiscard = useUnsavedChanges(humanDirty && action === null);
  function refreshWithGuard() { if (confirmDiscard()) void refresh(); }
  ...
  const missingOrderFields = reviewDraft ? [
    !reviewDraft.customer.name?.trim() && "联系人名称",
    !reviewDraft.customer.phone?.trim() && !reviewDraft.customer.wechat_name?.trim() && "联系方式",
    !reviewDraft.customer.address?.trim() && "详细地址",
    (!reviewDraft.cats.length || reviewDraft.cats.some((cat) => !cat.name?.trim())) && "猫咪名称",
    !hasServiceSchedule && "服务日期",
    !reviewDraft.service.service_items.length && "服务事项",
    (!unitPrice.trim() || !Number.isFinite(Number(unitPrice)) || Number(unitPrice) < 0) && "每次价格",
  ].filter(Boolean) : [];
  const orderReady = reviewSaved && missingOrderFields.length === 0;
  const orderHint = missingOrderFields.length ? `生成订单还需补齐：${missingOrderFields.join("、")}。` : "资料与价格已就绪，可以生成订单。";
  ...
  return (
    <section className={embedded ? "mt-6" : "cc-page cc-page--wide"} aria-labelledby="intake-title">
      {!embedded ? <nav className="cc-tabs" aria-label="订单工作区"><Link to="/admin/orders">订单与排班</Link><Link to="/admin/intake" aria-current="page">客户提交</Link></nav> : null}
      {embedded ? <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold tracking-wide text-brand-600">客户资料决策区</p><h2 id="intake-title" className="mt-1 text-xl font-semibold">客户填写、审核与归档</h2><p className="mt-1 text-sm text-slate-500">窗口可见时每 30 秒同步一次；云端异常不会影响上方工作台数据。</p></div><div className="flex gap-2"><Link to="/admin/intake" className="cc-button cc-button--secondary">查看全部历史</Link><button type="button" className="cc-button cc-button--secondary" onClick={refreshWithGuard} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} size={15} />刷新</button></div></div> : <PageHeader
        eyebrow="资料收集"
        title="客户填写"
        headingId="intake-title"
        description="生成专属链接、查看客户提交，并在人工复核后一次生成客户档案、已确认订单和每日任务。"
        actions={<><Link to="/admin/customers" className="cc-button cc-button--secondary">返回客户档案</Link><button type="button" className="cc-button cc-button--secondary" onClick={refreshWithGuard} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} size={15} />刷新</button></>}
      />}
