...
export function AdminIntakePage() {
  ...
  return (
    <section className="mx-auto max-w-[1500px]" aria-labelledby="intake-title">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="mb-1 text-xs font-semibold tracking-wider text-slate-500 uppercase">P10 · 客户填写入口</p><h1 id="intake-title" className="text-2xl font-semibold tracking-tight">客户填写</h1><p className="mt-2 text-sm leading-6 text-slate-600">生成专属链接、查看客户提交，并在人工确认后建立正式档案与待确认订单。</p></div>
        <div className="flex gap-2"><Link to="/admin/customers" className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">返回客户档案</Link><button type="button" className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => void refresh()} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} size={15} />刷新</button></div>
      </div>
      <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900"><strong>本地使用提示：</strong>当前尚未完成 P12 正式登录与权限隔离，只能在本机或可信局域网使用，不要直接暴露到公网。</div>
      {error ? <div className="mt-4 flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert"><AlertCircle className="mt-0.5 shrink-0" size={17} /><span>{error}</span></div> : null}

      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.35fr)]">
        <div className="space-y-6">
          <section className="rounded-xl border border-slate-200 bg-white p-5" aria-labelledby="new-link-title">
            <h2 id="new-link-title" className="flex items-center gap-2 font-semibold"><Link2 size={17} />生成填写链接</h2>
            <form className="mt-4 flex flex-wrap items-end gap-3" onSubmit={handleCreate}><label className="text-sm font-medium text-slate-700">有效天数<input className="mt-1.5 block w-32 rounded-md border border-slate-300 px-3 py-2 text-sm" type="number" min={1} max={90} value={expiresInDays} onChange={(event) => setExpiresInDays(Number(event.target.value))} required /></label><button type="submit" className="inline-flex items-center gap-2 rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50" disabled={action !== null}>{action === "create" ? <LoaderCircle className="animate-spin" size={15} /> : <Plus size={15} />}生成链接</button></form>
          </section>

          <section className="rounded-xl border border-slate-200 bg-white" aria-labelledby="links-title">
            <div className="border-b border-slate-200 px-5 py-4"><h2 id="links-title" className="font-semibold">填写链接（{tokens.length}）</h2></div>
            {loading ? <div className="flex items-center justify-center gap-2 py-12 text-sm text-slate-500"><LoaderCircle className="animate-spin" size={17} />正在加载…</div> : tokens.length === 0 ? <p className="px-5 py-10 text-center text-sm text-slate-500">还没有填写链接。</p> : <ul className="divide-y divide-slate-100">{tokens.map((token) => <li key={token.id} className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold">链接 #{token.id}</span><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${token.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{tokenStatusLabels[token.status]}</span>{token.submission_status ? <span className="rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">{submissionStatusLabels[token.submission_status]}</span> : null}</div><p className="mt-2 text-xs text-slate-500">有效期至 {dateTime(token.expires_at)}</p></div></div><div className="mt-3 flex flex-wrap gap-2"><a href={token.fill_path} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-medium"><ExternalLink size={13} />打开</a><button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-medium" onClick={() => void handleCopy(token)}><ClipboardCopy size={13} />{copiedId === token.id ? "已复制" : "复制"}</button>{!token.submitted_at && token.status !== "expired" ? <button type="button" className="rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-medium disabled:opacity-50" onClick={() => void handleTokenStatus(token)} disabled={action === `token-${token.id}`}>{token.status === "active" ? "关闭" : "恢复"}</button> : null}</div></li>)}</ul>}
          </section>
        </div>

        <section className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white" aria-labelledby="submissions-title">
          ...
          <div className="grid min-h-[600px] lg:grid-cols-[260px_minmax(0,1fr)]">
            <aside className="border-b border-slate-200 bg-slate-50/60 lg:border-r lg:border-b-0" aria-label="提交记录列表">{loading ? <div className="flex justify-center py-12"><LoaderCircle className="animate-spin text-slate-400" size={18} /></div> : submissions.length === 0 ? <p className="px-4 py-12 text-center text-sm text-slate-500">还没有客户提交。</p> : <ul className="p-2">{submissions.map((item) => <li key={item.id}><button type="button" className={`mb-1 w-full rounded-lg px-3 py-3 text-left ${selectedId === item.id ? "bg-slate-900 text-white" : "hover:bg-slate-100"}`} onClick={() => { setSelectedId(item.id); setConfirmingConvert(false); }}><div className="flex items-start justify-between gap-2"><span className="truncate text-sm font-semibold">{item.customer_name || "未填写姓名"}</span><span className={`shrink-0 text-[11px] ${selectedId === item.id ? "text-slate-300" : "text-slate-500"}`}>{submissionStatusLabels[item.status]}</span></div><p className={`mt-1 truncate text-xs ${selectedId === item.id ? "text-slate-300" : "text-slate-500"}`}>{item.community || "未填写小区"} · {item.cat_count} 只猫</p><p className={`mt-2 text-xs ${selectedId === item.id ? "text-slate-300" : "text-slate-500"}`}>{item.start_date || "日期未填"} 至 {item.end_date || "—"}</p></button></li>)}</ul>}</aside>
