      ...
      <div className="flex h-full flex-col gap-3 p-4">
        <div className="cc-surface p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${workspace?.transport_mode === "electrobike" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-600"}`}>路线方式：{workspace?.transport_mode === "electrobike" ? "电动自行车" : "状态未知"}</span>
            <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${workspace?.transport_mode === "electrobike" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-600"}`}>顺序建议：{workspace?.transport_mode === "electrobike" ? "本地快速" : "状态未知"}</span>
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500">
            订单保存时只把上门地址发送给地图服务完成定位；点击“生成路线”后才逐段请求电动车道路路线。姓名、电话、微信、门禁、钥匙和备注始终不会发送。
          </p>
          ...
          {workspace?.recommendation_message ? <p className={`mt-2 rounded-md px-3 py-2 text-xs leading-5 ${workspace.recommendation_source === "openai" ? "bg-emerald-50 text-emerald-800" : workspace.recommendation_source === "local" ? "bg-blue-50 text-blue-800" : "bg-amber-50 text-amber-800"}`}>{workspace.recommendation_message}</p> : null}
